...
FROM_EMAIL = os.getenv("EMAIL_FROM")  # the address you verified in Brevo
FROM_NAME = "deInfluence"
...
def send_otp_email(email: str, otp: str) -> bool:
    if not os.getenv("BREVO_API_KEY") or not FROM_EMAIL:
        logger.warning("⚠️  Brevo not configured")
    ...
    success = _send_email(email, "Your deInfluence OTP - Valid for 5 minutes", html_body)
    if success:
...
def send_welcome_email(email: str, name: str) -> bool:
    ...
    success = _send_email(email, "Welcome to deInfluence!", html_body)
    if success:
